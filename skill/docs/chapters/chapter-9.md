...
`npx odd-studio init` sets up everything: the /odd skill, six safety hooks, project scaffold, and odd-flow persistent memory. odd-flow stores all project state between sessions so you never have to reconstruct context from scratch.
...
- **odd-flow preserves the project state between sessions so you never have to reconstruct it.** Every outcome, every contract, every verification result, every decision recorded in CLAUDE.md — odd-flow keeps it all. When you start a new session, ODD Studio reads from odd-flow and picks up exactly where you left off. You do not re-explain your project.
...
- Attempting to manage project state manually. If you are keeping notes in a separate document, copying context into prompts, or re-explaining your project at the start of each session, you are doing odd-flow's job by hand. Let the tool manage state.
...
- Starting a build before init. If you begin building without running init, you have no hooks, no odd-flow memory, no project scaffold. You are building without a safety net or a state manager.

## What This Means for You

If you have not already, run `npx odd-studio init` in your project directory. Confirm that git is initialised, the skill is loaded, and odd-flow is connected. Then move to your Phase A outcomes.
